...
export const promptSchema = z.object({
  ...
  images: z.array(promptImageSchema).optional().describe("Optional array of images to include in the prompt"),
});
...
export const sourceSchema = z.object({
  ...
  ref: z
    ...
    .min(1)
    .default("main")
    .describe("Git ref (branch name, tag, or commit hash) to use as the base branch"),
});

// Webhook configuration for status notifications
export const webhookSchema = z.object({
  url: z.string().url().describe("URL to receive webhook notifications about agent status changes"),
  secret: z
    .string()
    .min(32)
    .optional()
    .describe("Secret key for webhook payload verification (minimum 32 characters)"),
});
...
export const targetSchema = z.object({
  autoCreatePr: z
    .boolean()
    .default(false)
    .describe("Whether to automatically create a pull request when the agent completes"),
  openAsCursorGithubApp: z
    .boolean()
    .default(false)
    .describe("Whether to open the pull request as the Cursor GitHub App instead of as the user. Only applies if autoCreatePr is true"),
  skipReviewerRequest: z
    .boolean()
    .default(false)
    .describe("Whether to skip adding the user as a reviewer to the pull request. Only applies if autoCreatePr is true and the PR is opened as the Cursor GitHub App"),
  branchName: z
    .string()
    .optional()
    .describe("Custom branch name for the agent to create. If not provided, a name will be auto-generated"),
});
...
export const launchAgentResponseSchema = z.object({
  ...
  createdAt: z.string().datetime().describe("ISO timestamp when the agent was created"),
  summary: z.string().optional().describe("Summary of what the agent accomplished"),
});
...
export const launchAgentFormSchema = launchAgentRequestSchema.extend({
  ...
  prompt: promptSchema.extend({
    text: z
      .string()
      .min(10, "Please provide a more detailed task description (at least 10 characters)")

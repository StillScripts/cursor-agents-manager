...
async function fetchApiKeyStatus(): Promise<ApiKeyResponse> {
  const response = await fetch("/api/user/openai-api-key")
  if (!response.ok) {
    throw new Error("Failed to fetch OpenAI API key status")
  }
...
async function saveApiKey(apiKey: string): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.error || "Failed to save OpenAI API key")
  }
}

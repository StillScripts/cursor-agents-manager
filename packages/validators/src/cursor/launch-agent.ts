...
export const sourceSchema = z.object({
  ...
  ref: z
    ...
    .optional()
    .describe(
      "Git ref (branch name, tag, or commit hash) to use as the base branch. If not provided, Cursor will use the default branch"
    ),
})
...
/**
 * Agent type derived from Cursor API response
 * This matches the format returned by the Cursor API
 */
export type Agent = z.infer<typeof launchAgentResponseSchema> & {
  audioSummary?: string // Base64 encoded audio data (added by our app)
}
...
export const launchAgentFormSchema = launchAgentRequestSchema.extend({
  ...
  taskId: z.string().optional().describe("Optional task ID to associate with this agent"),
})
